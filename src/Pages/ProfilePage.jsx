...
function ProfilePage() {
  ...
  // Track whether the current viewing user is verified
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      // Fetch the CURRENT LOGGED-IN VIEWER'S profile status to verify permissions
      if (sessionUser) {
  ...
  const initiateDirectCall = () => {
    // Only permit initiating calls if the dynamic validation rule passes
  ...
  // UPDATED: Only allow visibility if they own the profile OR they are a verified seller
        ...
        <div className="max-w-5xl mx-auto px-4 -mt-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 bg-white rounded-3xl p-6 shadow-xs border border-slate-100 flex flex-col items-center text-center h-fit">
              ...
              {/* Renders real details for verified viewers; hides it for unverified viewers */}
              <p className={`text-xs font-bold mt-1.5 px-3 py-1 rounded-lg ${canViewPhoneNumber ? 'text-slate-700 bg-slate-50' : 'text-rose-600 bg-rose-50'}`}>
                {displayPhoneNumber}
              </p>
