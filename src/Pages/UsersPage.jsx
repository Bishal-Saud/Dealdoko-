...
function UsersPage() {
  ...
  const fetchFilteredUsers = async () => {
    try {
      ...
      // Filter by location_name if specified, otherwise filter by text queries
      if (locationQuery.trim() !== '') {
        dbQuery = dbQuery.ilike('location_name', `%${locationQuery.trim()}%`);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {users.map((profile) => (
                <div key={profile.id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col h-full">
                  ...
                  {/* Profile Header with Rating and Verification Badge */}
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          ...
                          {profile.is_verified_seller && (
                            <ShieldCheck size={16} className="text-emerald-500 fill-emerald-500/10 shrink-0" />
                          )}
                        </div>
                        <p className="text-[10px] font-bold text-slate-400 flex items-center gap-1 mt-0.5">
                          <MapPin size={10} className="text-blue-500" /> {profile.location_name || "Nepal"}
                        </p>
                      </div>
                    </div>
